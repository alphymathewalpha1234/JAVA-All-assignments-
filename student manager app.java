...
class Student extends Person {
    ...
    public void inputDetails(Scanner sc) throws InvalidInputException {
        try {
            ...
            if (rline.isEmpty()) {
                throw new InvalidInputException("Roll no empty");
            }
            ...
            if (name.isEmpty()) {
                throw new InvalidInputException("Name empty");
            }

            System.out.print("Enter Email: ");
            email = sc.nextLine().trim();
            if (email.isEmpty()) {
                throw new InvalidInputException("Email empty");
            }

            System.out.print("Enter Course: ");
            course = sc.nextLine().trim();
            if (course.isEmpty()) {
                throw new InvalidInputException("Course empty");
            }

            System.out.print("Enter Marks: ");
            String mline = sc.nextLine().trim();
            if (mline.isEmpty()) {
                throw new InvalidInputException("Marks empty");
            }
            marks = Double.parseDouble(mline);
            if (marks < 0 || marks > 100) {
                throw new InvalidInputException("Marks must be 0-100");
            }
...
class StudentManager implements RecordActions {
    ...
    public synchronized void addStudent(Student s) throws InvalidInputException {
        if (s == null) {
            throw new InvalidInputException("Student null");
        }
        Integer key = Integer.valueOf(s.getRollNo());
        if (map.containsKey(key)) {
            throw new InvalidInputException("Duplicate roll no");
        }
    ...
    public synchronized void updateStudent(int rollNo, Student newData) throws StudentNotFoundException, InvalidInputException {
        ...
        if (newData == null) {
            throw new InvalidInputException("New data null");
        }
            ...
            switch (choice) {
                ...
                case 6:
                    try {
                        ...
                        if (existing == null) {
                            throw new StudentNotFoundException("Roll not found");
                        }
